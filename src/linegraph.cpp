...
LineGraph::LineGraph(QWidget *parent) : QMainWindow(parent),
                                        ui(new Ui::LineGraph)
{
    ui->setupUi(this);
...
void LineGraph::on_view_btn_clicked()
{
    ...
    // checking if the user selected income or expense
    QString income_or_expense, income_or_expense_;
    if (ui->select_income_btn->isChecked())
    {
        income_or_expense = temp_username + "_in";
        income_or_expense_ = "income";
    }
    ...
    {
        for (int i = 0; i <= 31; i++)
        {                               // storing 0 to 31 in x_days_numbers
    ...
    if (!ui->view_by_menu->currentIndex())
    { // to view graph on the basis of Transaction Amounts whose index in comboBox is 0, so !0 = 1
        ...
        QString amount_types[] = {"`Total amount`", income_or_expense_};
        for (int i = 0; i < 2; i++)
        { // to loop through amount_types array
            for (int day = 1; day <= last_day; day++)
            { // to loop through days
                ...
                qry->prepare("SELECT SUM (" + amount_types[i] + ") FROM " + income_or_expense + " WHERE Date LIKE " + date);
                qry->exec();
                model->setQuery(*qry);
        ...
        // to get maximum value among amounts
        double y_max_value;
        ...
        ui->customPlot->graph(1)->setName(income_or_expense_);

        ui->customPlot->addGraph();
        ui->customPlot->graph(2)->setPen(QPen(Qt::blue));
        ui->customPlot->graph(2)->setName(income_or_expense_);
    ...
    else if (ui->view_by_menu->currentIndex())
    { // to view graph on the basis of No. of transaction whose index in comboBox is 1
        db_conn_open();
        ...
        for (int day = 1; day <= last_day; day++)
        {
            ...
            { // if All is selected
                ...
                qry->prepare("SELECT COUNT (`Transaction ID`) FROM " + temp_username + "_ex WHERE Date LIKE " + date); // counting no. of row where given date matches
                qry->exec();
                model->setQuery(*qry);
                income_count = model->data(model->index(0, 0)).toDouble(); // returns value at index(0,0) i.e. first row, first column

                qry->prepare("SELECT COUNT (`Transaction ID`) FROM " + temp_username + "_in WHERE Date LIKE " + date); // counting no. of row where given date matches
                qry->exec();
                model->setQuery(*qry);
                income_count = model->data(model->index(0, 0)).toDouble();

                num_of_transactions_list.push_back(income_count + expense_count); // number of transaction including both buyers and sellers
            }
            else if (ui->select_income_btn->isChecked() || ui->select_expense_btn->isChecked())
            {                                                                                                           // if Buyer or Seller is selected
                qry->prepare("SELECT COUNT (`Transaction ID`) FROM " + income_or_expense + " WHERE Date LIKE " + date); // add date
...
{
    if (!index)
    { // only option for Buyer and Seller is shown, option for All is not shown
        ui->select_all_btn->setVisible(0);
    }
    else if (index)
    { // option of Buyer, Seller and All is shown
